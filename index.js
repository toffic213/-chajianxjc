(() => {
  ...
  function fire() { if (initialized) return; initialized = true; try { init(); } catch (error) { initialized = false; console.warn('[stage-theater] init failed', error); } }
  try {
  ...
  const interval = window.setInterval(() => { if (window.SillyTavern || window.extension_settings || Date.now() - startedAt > 3500) { window.clearInterval(interval); fire(); } }, 250);
})();
