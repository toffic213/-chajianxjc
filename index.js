(() => {
  ...
  console.info(`[${PLUGIN_ID}] index.js loaded (v${PLUGIN_VERSION})`);
  ...
  function getContext() {
    try {
      ...
      return st && typeof st.getContext === 'function' ? (st.getContext() || {}) : {};
    } catch (error) { return {}; }
  }
  ...
  function readSettings() {
    ...
    const tavern = (window.extension_settings && window.extension_settings[PLUGIN_ID]) || {};
  ...
  function lastAiMessage() {
    ...
    for (let i = messages.length - 1; i >= 0; i -= 1) {
      ...
      if (message && (message.is_user || message.role === 'system')) continue;
      const text = String(message && (message.mes != null ? message.mes : message.content != null ? message.content : '')).trim();
      if (text) return text;
    }
    return '';
  }
  ...
  function contextText() {
    ...
    return chatMessages().slice(-Math.max(1, Number(settings.contextDepth) || 1)).map((message) => {
      const role = message && message.is_user ? '用户' : message && message.role === 'system' ? '系统' : 'AI';
      return `${role}: ${String(message && (message.mes != null ? message.mes : message.content != null ? message.content : '')).trim()}`;
    }).filter(Boolean).join('\n');
  }

  function worldbookText() {
    if (!settings.includeWorldbook) return '';
    const metadata = ctx && ctx.chatMetadata;
    const candidates = [ctx && ctx.worldInfo, ctx && ctx.worldbook, ctx && ctx.worldBook, metadata && metadata.worldInfo, metadata && metadata.worldbook, window.worldInfo];
    for (const candidate of candidates) {
      ...
      } catch (error) { /* ignore incompatible worldbook objects */ }
    }
    return '';
  }
  ...
  async function generateTheater(force = false) {
    ...
    try {
      ...
      setStatus((error && error.message) || '生成失败，请检查 API 设置。');
    } finally { generationBusy = false; }
  }

  function setStatus(text) {
    const element = root && root.querySelector('[data-stt-status]');
    if (element) element.textContent = text;
  }

  function positionPanel() {
    const fab = root && root.querySelector('.stt-fab');
    const panel = root && root.querySelector('[data-stt-panel]');
    if (!fab || !panel) return;
  ...
  function renderOutput(text) {
    const element = root && root.querySelector('[data-stt-output]');
  ...
  function renderScenes() {
    const list = root && root.querySelector('[data-stt-scenes]');
  ...
  function syncSettingsUi() {
    ...
    root.querySelectorAll('[data-setting]').forEach((element) => {
      ...
      else element.value = settings[key] == null ? '' : settings[key];
    });
    renderScenes();
  }
  ...
  function deleteScene(element) {
    const holder = element && element.closest('[data-scene-id]');
    const id = holder ? holder.dataset.sceneId : null;
  ...
  function importPresetValue(value) {
    const entries = Array.isArray(value) ? value : value && Array.isArray(value.prompts) ? value.prompts : value && Array.isArray(value.entries) ? value.entries : value && Array.isArray(value.items) ? value.items : [value];
  ...
  function importPreset() {
    const fileInput = root && root.querySelector('[data-stt-preset-file]');
    ...
    } catch (error) { setStatus(error && error.message || '预设 JSON 格式不正确。'); }
  }

  function pastePreset() {
    const raw = window.prompt('粘贴预设条目 JSON（单个对象或数组，字段可用 name/prompt/content）');
    if (!raw) return;
    try { importPresetValue(JSON.parse(raw)); } catch (error) { setStatus(error && error.message || '预设 JSON 格式不正确。'); }
  }
  ...
  function bindUi() {
    ...
    root.addEventListener('change', (event) => {
      ...
      if (event.target.matches('[data-action="toggle-scene"]')) {
        const holder = event.target.closest('[data-scene-id]');
        const id = holder ? holder.dataset.sceneId : null;
      ...
      if (event.target.matches('[data-stt-preset-file]')) {
        ...
        reader.onload = () => { try { importPresetValue(JSON.parse(String(reader.result || ''))); } catch (error) { setStatus(error && error.message || '预设 JSON 格式不正确。'); } };
  ...
  function bindDrag(fab) {
    ...
    fab.addEventListener('pointerdown', (event) => { pointer = { id: event.pointerId, x: event.clientX, y: event.clientY, moved: false }; if (fab.setPointerCapture) fab.setPointerCapture(event.pointerId); });
    ...
    fab.addEventListener('pointerup', () => { suppressClick = Boolean(pointer && pointer.moved); pointer = null; });
    fab.addEventListener('click', (event) => { if (suppressClick) { event.preventDefault(); event.stopPropagation(); suppressClick = false; } });
  }

  // 这个内联诊断球不依赖 style.css / 酒馆上下文：只要 index.js 被加载，页面就应能看到它。
  function mountDiagnosticFab(error) {
    if (!document.body || document.getElementById(ROOT_ID) || document.getElementById('stt-diagnostic-fab')) return;
    const button = document.createElement('button');
    button.id = 'stt-diagnostic-fab';
    button.type = 'button';
    button.textContent = '剧';
    button.title = '小剧场脚本已加载，但初始化失败；点击查看错误';
    button.style.cssText = 'position:fixed;right:18px;bottom:72px;z-index:2147483647;width:48px;height:48px;border-radius:50%;border:2px solid #fff;background:#b33;color:#fff;font-size:20px;font-weight:700;box-shadow:0 3px 12px #000;';
    button.addEventListener('click', () => window.alert(`小剧场已加载，但初始化失败：${error && error.message || error || '未知错误'}\n请打开酒馆控制台查看 [stage-theater] 日志。`));
  ...
  function mount() {
    ...
    console.info(`[${PLUGIN_ID}] floating button mounted`);
    const visibleFab = root.querySelector('.stt-fab');
    if (visibleFab) visibleFab.style.cssText = 'position:fixed;right:20px;bottom:84px;z-index:2147483646;width:52px;height:52px;border:2px solid #fff;border-radius:50%;background:#2c9b7d;color:#fff;cursor:pointer;display:grid;place-items:center;box-shadow:0 8px 24px rgba(0,0,0,.45);touch-action:none;';
    const diagnostic = document.getElementById('stt-diagnostic-fab');
    if (diagnostic) diagnostic.remove();
    syncSettingsUi();
    bindUi();
    const metadata = ctx && ctx.chatMetadata;
    const extensions = metadata && metadata.extensions;
    const pluginData = extensions && extensions[PLUGIN_ID];
    const imported = pluginData && pluginData.lastOutput;
  ...
  function init() {
    ...
    const eventName = types.MESSAGE_RECEIVED != null ? types.MESSAGE_RECEIVED : types.MESSAGE_UPDATED != null ? types.MESSAGE_UPDATED : types.GENERATION_STARTED;
    if (source && typeof source.on === 'function' && eventName != null) source.on(eventName, onMessageReceived);
  }

  let initialized = false;
  function fire() {
    if (initialized || !document.body) return;
    try { init(); initialized = true; }
    catch (error) { console.warn('[stage-theater] init failed; retrying', error); mountDiagnosticFab(error); }
  }
  try {
    const early = getContext();
    if (early.eventSource && early.event_types && early.event_types.APP_READY != null) early.eventSource.on(early.event_types.APP_READY, fire);
  } catch (error) { /* polling fallback below */ }
